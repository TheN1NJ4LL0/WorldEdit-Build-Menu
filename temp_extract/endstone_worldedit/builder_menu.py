...
class MenuHandler:
    ...
    def show_copy_options_form(self, player: "Player") -> None:
        ...
        form.add_toggle("Include Air Blocks", False)
        form.add_toggle("Copy Entities", False)
        form.add_toggle("Copy Biomes", False)
        ...
        def on_submit(player: "Player", data: Optional[list]):
            ...
            include_air = data[0] if len(data) > 0 else False
            copy_entities = data[1] if len(data) > 1 else False
            copy_biomes = data[2] if len(data) > 2 else False
    ...
    def show_set_blocks_form(self, player: "Player") -> None:
        ...
        form.add_input("Block Type:", "Example: stone, grass, diamond_block", "stone")
    ...
    def show_replace_blocks_form(self, player: "Player") -> None:
        ...
        form.add_input("From Block:", "Block to replace", "stone")
        form.add_input("To Block:", "New block type", "grass")

        def on_submit(player: "Player", data: Optional[list]):
            if data is None or not data[0] or not data[1]:
                self.show_editing_menu(player)
                return

            from_block = data[0].strip()
            to_block = data[1].strip()
    ...
    def show_walls_form(self, player: "Player") -> None:
        ...
        form.add_input("Block Type:", "Example: stone, brick", "stone")
    ...
    def show_overlay_form(self, player: "Player") -> None:
        ...
        form.add_input("Block Type:", "Example: grass, snow", "grass")
    ...
    def show_sphere_form(self, player: "Player", hollow: bool) -> None:
        ...
        form.add_input("Block Type:", "Example: stone, glass", "stone")
        form.add_input("Radius:", "Sphere radius", "5")
    ...
    def show_cylinder_form(self, player: "Player", hollow: bool) -> None:
        ...
        form.add_input("Block Type:", "Example: stone, glass", "stone")
        form.add_input("Radius:", "Cylinder radius", "5")
        form.add_input("Height:", "Cylinder height", "10")

        def on_submit(player: "Player", data: Optional[list]):
            if data is None or not data[0] or not data[1] or not data[2]:
                self.show_shapes_menu(player)
                return
    ...
    def show_save_schematic_form(self, player: "Player") -> None:
        ...
        form.add_input("Filename:", "Without .schem extension", "my_build")
        form.add_toggle("Include Air Blocks", False)
        ...
        def on_submit(player: "Player", data: Optional[list]):
            ...
            include_air = data[1] if len(data) > 1 else False
    ...
    def show_load_and_place_form(self, player: "Player") -> None:
        ...
        # Rotation
        form.add_dropdown("Rotation:", ["0° (No rotation)", "90° (Clockwise)", "180°", "270° (Counter-clockwise)"], 0)

        # Flip options
        form.add_toggle("Flip X (East/West)", False)
        form.add_toggle("Flip Y (Up/Down)", False)
        form.add_toggle("Flip Z (North/South)", False)

        # Placement position
        form.add_dropdown("Place At:", ["Current Position", "In Front (3 blocks)", "In Front (5 blocks)", "In Front (10 blocks)", "Custom Offset"], 0)

        # Custom offset (only used if "Custom Offset" selected)
        form.add_input("Custom Offset X:", "East(+) / West(-)", "0")
        form.add_input("Custom Offset Y:", "Up(+) / Down(-)", "0")
        form.add_input("Custom Offset Z:", "South(+) / North(-)", "0")

        # Additional options
        form.add_toggle("Include Air Blocks", True)
        form.add_toggle("Paste Entities", True)
        form.add_toggle("Paste Biomes", False)
        ...
        def on_submit(player: "Player", data: Optional[list]):
            ...
            try:
                ...
                # Parse rotation
                rotation = data[1] if len(data) > 1 else 0
                rotation_degrees = [0, 90, 180, 270][rotation]

                # Parse flips
                flip_x = data[2] if len(data) > 2 else False
                flip_y = data[3] if len(data) > 3 else False
                flip_z = data[4] if len(data) > 4 else False

                # Parse placement position
                placement = data[5] if len(data) > 5 else 0

                # Parse custom offsets
                custom_x = int(data[6]) if len(data) > 6 and data[6] else 0
                custom_y = int(data[7]) if len(data) > 7 and data[7] else 0
                custom_z = int(data[8]) if len(data) > 8 and data[8] else 0
                ...
                include_air = data[9] if len(data) > 9 else True
                paste_entities = data[10] if len(data) > 10 else True
                paste_biomes = data[11] if len(data) > 11 else False
    ...
    def show_paste_options_form(self, player: "Player") -> None:
        ...
        form.add_input("Offset X (blocks):", "East(+) / West(-)", "0")
        form.add_input("Offset Y (blocks):", "Up(+) / Down(-)", "0")
        form.add_input("Offset Z (blocks):", "South(+) / North(-)", "0")

        # Additional options
        form.add_toggle("Include Air Blocks", False)
        form.add_toggle("Paste Entities", False)
        form.add_toggle("Paste Biomes", False)
        ...
        def on_submit(player: "Player", data: Optional[list]):
            ...
            try:
                # Parse rotation
                rotation = data[0] if len(data) > 0 else 0
                rotation_degrees = [0, 90, 180, 270][rotation]

                # Parse flips
                flip_x = data[1] if len(data) > 1 else False
                flip_y = data[2] if len(data) > 2 else False
                flip_z = data[3] if len(data) > 3 else False

                # Parse offsets
                offset_x = int(data[4]) if len(data) > 4 and data[4] else 0
                offset_y = int(data[5]) if len(data) > 5 and data[5] else 0
                offset_z = int(data[6]) if len(data) > 6 and data[6] else 0

                # Parse additional options
                include_air = data[7] if len(data) > 7 else False
                paste_entities = data[8] if len(data) > 8 else False
                paste_biomes = data[9] if len(data) > 9 else False
